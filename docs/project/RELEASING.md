...
3. writes `update.json`, the self-update manifest naming each macOS and Linux archive with its SHA-256 from `SHA256SUMS` (`automation/release/gen-update-manifest.py --sha256sums`), signs it with the Ed25519 key in the `release` environment secret `PIG_UPDATE_SIGNING_KEY`, verifies the signature against `automation/release/update-trust.pem`, and attaches both `update.json` and `update.json.sig`. The job fails if the secret is missing;
...
To set up or rotate the signing key: generate an Ed25519 key (`openssl genpkey -algorithm ed25519 -out update-signing.pem`), store its PEM as the `PIG_UPDATE_SIGNING_KEY` secret of the `release` environment, and commit its public key (`openssl pkey -in update-signing.pem -pubout`) to `automation/release/update-trust.pem`. For a rotation, keep the outgoing public key in that file beside the incoming one for at least one release, so binaries built with either key keep verifying.
