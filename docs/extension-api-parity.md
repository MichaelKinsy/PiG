...
| `getArgumentCompletions` (command option) | the editor asks the extension for a command's argument completions off the TUI loop after "/<command> " and shows them when they arrive; Enter accepts the highlighted argument without submitting. Node extensions and the Go (`RegisterCommand` with `CommandOptions.GetArgumentCompletions`), Rust (`command_argument_completions`) and Python (`command(..., get_argument_completions=)`) SDKs over `command_argument_completions`; in-process Go commands directly | `TestNodeCommandArgumentCompletionsRunInTheExtensionProcess`, the `argument_completions` row of `TestConformance_TransportsMatch`, `TestExtensionCommandArgumentCompletionAcceptsWithoutSubmitting`, `extensions-runtime/27-command-argument-completions` | complete |
...
| `renderCall` | the tool card runs it on every card state change, with one renderer state per card shared with `renderResult`, the renderer's last component, and `context.invalidate` rerunning both. A Node extension returns its own component; the host requests `render_tool` off the TUI loop and keeps the last frame (D56). Go/Rust/Python renderers return lines. A renderer that throws draws upstream's fallback. HTML export (`/export`, RPC `export_html`) runs the renderers as upstream's export does, asking an extension process for its frame and waiting up to the renderer inactivity boundary. An override of a built-in tool gets the built-in renderer for any half it does not define (upstream `withBuiltInRenderers`). | `TestNodeToolRenderersRunInTheExtensionProcess`, `TestToolRenderersAcrossSDKs`, the `tool_renderer` row of `TestConformance_TransportsMatch`, `TestDefinitionCard*`, `TestInteractiveModeDrawsToolDefinitionRenderers`, `TestBuiltInOverride*`, `TestBuiltInEditRenderersPreviewAndResultDiff`, and `extensions-runtime/24-tool-renderers-collapsed` and `25-tool-renderers-expanded` | complete |
