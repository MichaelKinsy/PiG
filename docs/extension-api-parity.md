...
| `getArgumentCompletions` (command option) | the editor asks the extension for a command's argument completions off the TUI loop after "/<command> " and shows them when they arrive; Enter accepts the highlighted argument without submitting. Node extensions over `command_argument_completions`; in-process Go commands directly; the Go, Rust and Python SDKs do not declare it yet | `TestNodeCommandArgumentCompletionsRunInTheExtensionProcess`, `TestExtensionCommandArgumentCompletionAcceptsWithoutSubmitting`, `extensions-runtime/27-command-argument-completions` | partial |
