...
| D73 | Host-bound Pi exports in extensions | Extensions can import every value Pi's packages export. Pure helpers work as in Pi. Values that belong to Pi's own process (its UI components, session and runtime construction, package loading, terminal images, pi-ai's provider layer) throw a named error if an extension calls them. `parseFrontmatter` supports flat `key: value` frontmatter. |
